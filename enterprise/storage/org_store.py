...
class OrgStore:
    ...
    def _merge_and_validate_settings(
        ...
        is merged. Legacy persisted org rows may still carry
        ``agent_kind: 'llm'``; normalize only that stored value while leaving
        explicit incoming diffs to validate as written.
        """
        if settings_type is OpenHandsAgentSettings:
            ...
            if (
                (current_settings or {}).get('agent_kind') == 'llm'
